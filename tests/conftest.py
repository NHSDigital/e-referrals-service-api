...
async def user_restricted_app(client, make_app, user_restricted_product, asid):
    # Setup
    appName = await make_app(user_restricted_product, {"asid": asid})
...
def make_app(client):
    async def _make_app(product, custom_attributes):
        ...
        app.create_app(email="apm-testing-internal-dev@nhs.net", body=body)
...
@pytest.fixture
def authenticate_user(client, user_restricted_app, environment):
    async def _auth(actor: Actor):
        print(f"Attempting to authenticate: {actor}")

        app = DeveloperAppsAPI(client=client)
        full_app = app.get_app_by_name(
            email="apm-testing-internal-dev@nhs.net", app_name=user_restricted_app
        )

        credentials = full_app["credentials"][0]
        ...
        config = AuthorizationCodeConfig(
            ...
            callback_url=full_app["callbackUrl"],
        )
...
async def app_restricted_access_code(
    client, app_restricted_app, jwt_private_key_pem, environment
):
    app = DeveloperAppsAPI(client=client)
    full_app = app.get_app_by_name(
        email="apm-testing-internal-dev@nhs.net", app_name=app_restricted_app
    )
