...
ORIGINAL_HEADERS = {
    ...
    OBO_USER_ID: "nhsd-ers-on-behalf-of-user-id",
}
...
class TestHeaders:
    ...
    def test_headers(self, access_code, service_url):
        client_request_headers = {
            ...
            ORIGINAL_HEADERS[OBO_USER_ID]: EXPECTED_OBO_USER_ID,
        }
        ...
        assert (
            target_request_headers[RENAMED_HEADERS[REFERRAL_ID]] == EXPECTED_REFERRAL_ID
        )
        assert target_request_headers[RENAMED_HEADERS[CORRELATION_ID]].startswith(
            EXPECTED_CORRELATION_ID
        )
        assert (
            target_request_headers[RENAMED_HEADERS[BUSINESS_FUNCTION]]
            == EXPECTED_BUSINESS_FUNCTION
        )
        ...
        assert (
            target_request_headers[RENAMED_HEADERS[COMM_RULE_ORG]]
            == EXPECTED_COMM_RULE_ORG
        )
        assert (
            target_request_headers[RENAMED_HEADERS[OBO_USER_ID]] == EXPECTED_OBO_USER_ID
        )
